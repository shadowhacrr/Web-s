...
bot.on('text', async (ctx) => {
  ...
  // Cancel
  if (txt === '/cancel') {
    if (scrp.active.has(uid)) { scrp.active.delete(uid); return ctx.reply('❌ Cancelled.'); }
    if (owner.sessions.has(uid)) { owner.sessions.delete(uid); return ctx.reply('❌ Cancelled.'); }
  ...
  if (scrp.active.has(uid)) {
    ...
    if (s.status === 'waiting') return scrp.processUrl(ctx, txt);
  }

  // Owner input
  const ses = owner.sessions.get(uid);
  if (ses && owner.isOwner(ctx)) {
...
async function main() {
  console.log('');
  console.log('╔══════════════════════════════════════╗');
  console.log('║   ★ SHADOW WEB SCRAPER BOT ★       ║');
  console.log('╚══════════════════════════════════════╝');
  console.log('');
